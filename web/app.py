...
def line_chart():
    ...
    if len(s_datasets_dicts) == 1:
        return render_template(
            'line_chart.html',
            values1=list(s_datasets_dicts.values())[0].values,
            legend1=list(s_datasets_dicts.values())[0].legend,
            labels=x,
            critical_logs=c_log,
            error_logs=e_log,
            warning_log=w_log,
        ...
        return render_template(
            ...
            legend1=list(s_datasets_dicts.values())[0].legend,
            labels=x,
            values2=list(s_datasets_dicts.values())[1].values,
            legend2=list(s_datasets_dicts.values())[1].legend,
            critical_logs=c_log,
            error_logs=e_log,
            warning_logs=w_log,
            )
...
if __name__ == '__main__':
    parser = \
        argparse.ArgumentParser(description='Webserver for LiDAR temperature logs.'
                                )
    parser.add_argument('-f', '--file', required=True, type=str,
                        help='The path to LiDAR temperature logs.')
    parser.add_argument('-n', '--num', type=int, help='Num of LiDARs',
                        default=2)
    ...
    e_log = []
    w_log = []
    s_datasets_dicts['201'] = \
        Sensor_datasets(legend='{} {} {}'.format('LiDAR', '201',
                        'voltage'), values=[], labels=[])
    if args.num >= 2:
        s_datasets_dicts['202'] = \
            Sensor_datasets(legend='{} {} {}'.format('LiDAR', '202',
                            'voltage'), values=[], labels=[])
    with open(args.file) as f:
        lines = f.read().splitlines()
        pattern = \
            re.compile(r'([0-9]+-[0-9]+-[0-9]+)\s([0-9]+:[0-9]+:[0-9]+,[0-9]+)\s(-.*-)\s(.*?):\s(.*?):([0-9]+)\s(.*?):([0-9]+\.[0-9]*)'
                       )
        for l in lines:
            ...
            if s_id not in s_datasets_dicts:
                s_datasets_dicts[s_id] = \
                    Sensor_datasets(legend='{} {} {}'.format(s_type,
                                    s_id, s_p), values=[], labels=[])
            x.append(date + ' ' + time)
